...
import { postVanguardBrief, VanguardBriefResponse, VanguardSource } from "../../api/vanguard";

export default function VanguardWidget() {
  const [query, setQuery] = useState("SSR vs SSG para dashboard (Next.js/React)");
  const [sourcesJson, setSourcesJson] = useState<string>(() =>
    JSON.stringify(
      [
        { title: "Next.js 15 release notes", url: "https://nextjs.org/blog/next-15", date: "2025-05-15", domain: "nextjs.org" },
        { title: "React 19 RC", url: "https://react.dev/blog/2025/rc", date: "2025-06-10", domain: "react.dev" },
        { title: "Vite 6 perf guide", url: "https://vite.dev/perf", date: "2025-04-02", domain: "vite.dev" },
      ],
      null,
      2
    )
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        <strong className="text-xs uppercase tracking-wide opacity-70">Vanguarda</strong>
      ...
      <label className="block text-xs opacity-70">Pergunta (engenharia-only)</label>
      <input
      ...
      />
      <label className="block text-xs opacity-70">Fontes (JSON opcional: url+date obrigatórios)</label>
      <textarea
      ...
      {resp ? (
        <div className="space-y-2">
          ...
          <div className="text-xs opacity-70">
            Validação: {resp.validation.ok ? "OK" : `Falhou (${resp.validation.reasons.join(", ")})`} · Gate: {resp.gate.status}
          </div>
        </div>
      ) : null}
    </div>
