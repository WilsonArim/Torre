...
export default function KpiBadge() {
  ...
  const modeColor = (b.golden_sr ?? 0) >= 95 ? "bg-emerald-600" : "bg-amber-600";
  ...
  return (
    <div className="inline-flex items-center gap-2" title={err ?? title} aria-live="polite" aria-label="KPIs badge">
      <span className={`px-2 py-0.5 rounded-full text-[11px] text-white ${modeColor}`}>
        KPIs: {fmtPct(b.golden_sr)} / {fmtPct(b.repeat_error_rate)}
      </span>
      <button
        className="text-[11px] underline decoration-dotted"
        onClick={async () => { try { await exportKpis(); } catch {} }}
