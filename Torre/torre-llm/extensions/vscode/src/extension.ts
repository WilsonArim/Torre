...
export function activate(context: vscode.ExtensionContext) {
  context.subscriptions.push(
    vscode.commands.registerCommand("fortaleza.patch", async () => {
      ...
      if (!editor) {
        vscode.window.showWarningMessage("Abra um ficheiro.");
        return;
      }
      ...
      const body = {
        ...
        files,
        context: {
          ide: vscode.env.appName.toLowerCase().includes("cursor")
            ? "cursor"
            : "vscode",
          diagnostics,
        },
      ...
      let res;
      try {
        res = await fetch(`${apiUrl}/editor/patch`, {
          method: "POST",
          headers,
        ...
        vscode.window.showErrorMessage(`Fortaleza: erro de rede: ${message}`);
        return;
      }
      ...
      if (
        json.mode === "PATCH" &&
        json.files_out &&
        Object.keys(json.files_out).length
      ) {
        await applyFilesOut(json.files_out);
        vscode.window.showInformationMessage(
          "Fortaleza: alterações aplicadas.",
        );
      } else if (json.diff) {
        ...
        vscode.window.showInformationMessage(
          "Fortaleza: diff gerado (revise e aceite).",
        );
      } else {
        vscode.window.showWarningMessage(
          "Fortaleza: sem patch seguro — modo Advisory.",
        );
      }
    }),
