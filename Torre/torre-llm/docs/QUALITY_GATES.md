...
**Objetivo:** elevar padrão de _segurança, arquitetura, performance e duplicação_ sem travar fluxo quando não é crítico.
...
1. **Segurança (block)**
   ...
   - Referência a paths sensíveis (.env, .ssh, id*rsa, *.pem, secrets.\_).
2. **Arquitetura (advisory)**
   - UI/Components importando infra/db diretamente.
3. **Performance (advisory)**
   - Heurística de _nested loops_ ou `map().map()` na mesma região alterada.
