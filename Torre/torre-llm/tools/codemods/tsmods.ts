...
export async function missingSymbolImport(project: Project, file: string): Promise<Result> {
  ...
  if (imports.length > 0) {
    ...
    if (firstStatement && firstStatement.getKind() !== SyntaxKind.ImportDeclaration) {
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  Promise.all(tasks).then((results) => {
    ...
    console.log(JSON.stringify({ ok:true, files: files.length, edits_total, per_codemod: per }, null, 2));
  });
}
