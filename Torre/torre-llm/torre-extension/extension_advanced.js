...
function activate(context) {
    ...
    async function activateModel(modelName, modelId) {
        try {
            ...
            if (health.status === 'ok') {
                vscode.window.showInformationMessage(`🏰 ${modelName} ativado! API conectada.`);
                console.log(`🏰 Modelo ${modelName} ativado com sucesso`);
            } else {
                vscode.window.showWarningMessage(`🏰 ${modelName} ativado! API não disponível.`);
            }
        } catch (error) {
            console.error(`Erro ao ativar ${modelName}:`, error);
            vscode.window.showErrorMessage(`Erro ao ativar ${modelName}: ${error.message}`);
        }
    }
    ...
    async function autoFixCode() {
        try {
            ...
            if (result.diff) {
                ...
                workspaceEdit.replace(uri, new vscode.Range(0, 0, document.lineCount, 0), result.files_out[filename] || content);
    ...
    let enableAdvice = vscode.commands.registerCommand('torre.enableAdvice', () => {
        activateModel('Torre Advice', 'torre-advice');
    });
    
    let enableReview = vscode.commands.registerCommand('torre.enableReview', () => {
        activateModel('Torre Review', 'torre-review');
    });
    
    let enableExplain = vscode.commands.registerCommand('torre.enableExplain', () => {
    ...
    let disableAll = vscode.commands.registerCommand('torre.disableAll', () => {
        currentModel = null;
        statusBarItem.text = "🏰 Torre";
        vscode.window.showInformationMessage('🏰 Todos os modelos da Torre desativados');
    });
    
    // Adicionar comandos
    context.subscriptions.push(
        enableAuto, enableBase, enableAdvice, enableReview, enableExplain, 
        autoFix, disableAll
    );
