  ...
  for (const [file, froms] of symbolToFileMap.entries()) {
    ...
    for (const { symbol, from } of froms) {
      ...
      const has = sf
        ...
        .some((d) => d.getNamedImports().some((ni) => ni.getName() === symbol));
      if (!has) {
        setFromImport(sf, symbol, module.replace(/\.(tsx|ts|jsx|js)$/, ""));
        changed = true;
      }
    }
  }
...
const candidateFiles = await glob("src/**/*.{ts,tsx}", {
  withFileTypes: false,
});
...
if (names.length) {
  project.getSourceFiles().forEach((sf) => {
    if (prefixUnusedInFile(sf, names)) anyChange = true;
  });
}
// 2) addImport
...
if (ops.some((o) => o.type === "fixImportPath")) {
  project.getSourceFiles().forEach((sf) => {
    if (fixImportPath(sf)) anyChange = true;
  });
}
